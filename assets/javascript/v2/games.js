...
async function loadData() {
  let allData = await Promise.all([fetchGameData(), fetchPurchaseData(), fetchLessonData(), fetchSystemData()]);
  ...
  for(let data of allData) {
    if (data.type == "game") {
      ...
      // allSystems = data.systems.map(system => new System(system));
    }

    dates.push(data.modified);
  }

  parseGames(gameStatuses);
  parsePurchases(purchaseCategories);
  parseLessons();

  updateModified(dates);
  restoreView();
}
...
function parsePurchases(categories) {
  for (let currentCategory of categories) {
    ...
    if (div != null) {
      ...
      for(let purchase of purchases) {
        let completed = !purchase.isWaiting
...
function sortGames(games, status) {
  if (["unplayed", "vr"].includes(status)) {
...
function compareWithRelease(a, b) {
  let gameReleaseA = new Date(a.releases).getTime();
  let gameReleaseB = new Date(b.releases).getTime();
...
// Compare by recently added, newest to oldest
...
function purchaseToHTML(purchase, completed) {
  ...
  if (purchase.hasRelease) {
    output.push(` [${shortDate(purchase.releases)}]`);
